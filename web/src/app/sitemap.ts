...
const STATIC_ROUTES: Array<{
  ...
  { path: "/eliminar-datos", priority: 0.3, changeFrequency: "yearly" },
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [municipios, veredas, stories] = await Promise.all([
    prisma.municipio.findMany({ select: { divipolaCode: true } }),
    prisma.vereda.findMany({ select: { slug: true, municipio: { select: { divipolaCode: true } } } }),
  ...
  return [
    ...
    ...veredas.flatMap((v) =>
      entry(`/ciudad/${v.municipio.divipolaCode}/${v.slug}`, {
        priority: 0.5,
        changeFrequency: "weekly",
      }),
    ),
    ...
    // Docs (/docs/**) are fully bilingual (see lib/source.ts) with the same
    // locale-agnostic path on both sides (es unprefixed, en at /en/docs/**),
    // so these reuse entry() exactly like every static route above.
    ...docsSource
      .getPages("es")
      .flatMap((page) =>
        entry(`/docs${page.slugs.length ? `/${page.slugs.join("/")}` : ""}`, {
          priority: 0.5,
          changeFrequency: "monthly",
        }),
      ),
