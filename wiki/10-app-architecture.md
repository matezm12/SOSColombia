...
| **2 — structured but no API** | UNGRD balances, OCHA Flash Updates, INMLCF Comunicados | Scheduled job checks known URL patterns for new numbered bulletins (Comunicado 07, Flash Update 005, etc.), parses, inserts as new `TollRecord` rows — **never overwrites**. Needs a human glance before publish early on (bulletin formats aren't perfectly consistent) but can tighten to fully automatic once a few cycles prove the parser stable. | Daily during active response, tapering as the event ages |
