...
| **2 — structured but no API** | UNGRD balances, OCHA Flash Updates, INMLCF Comunicados (national/department level only) | Built 2026-08-14: `web/src/app/api/cron/bulletins` detects a newer numbered bulletin, stages a `PendingTollRecord` stub (no figures yet — formats aren't consistent enough to parse unattended) and emails an alert. A moderator reads the actual bulletin and fills in metric/value/asOf at `/admin/boletines`, which is what creates the real `TollRecord` row — **never overwrites**. Extended 2026-08-15: `web/src/app/api/cron/gov-news-check` (built for tier 3, see below) also runs a death-toll keyword pass over the same city press pages it already fetches, staging city-scoped `PendingTollRecord` rows the same way — this is what tier-2's national-only bulletins can't cover, since INMLCF/OCHA don't publish per-city figures. Learned the hard way that a wide context window on Pereira's unpaginated multi-year archive page produces false positives (matched "muerte" in an unrelated 2022 cancer-awareness article) — tightened to a narrower window, same discipline either way: detect only, never auto-write. | Daily during active response, tapering as the event ages |
| **3 — social/hyperlocal aid-point status** | Alcaldía social posts, crowd reports (Coliseo del Sur-style) | This is the hardest tier — no API, format varies, and it's exactly the data that changes fastest ("shelter now full," "new collection point opened"). Two complementary approaches, not either/or: (a) **downscoped 2026-08-14** — the original plan (browser-automation sweep of official *social* accounts) hit a real wall: a live probe found Facebook serves unauthenticated fetch() only page-level SEO metadata, zero real post content, and X/TikTok's oEmbed only renders a post you already have the URL for, not discovery of new ones. Neither is automatable without paid Meta/X API access. Downscoped to each municipio's own *government press-release page* instead — plain server-rendered HTML for 5 of 8 tracked cities (confirmed by direct fetch: Pereira, Cali, Manizales, Armenia, Dosquebradas; Quibdó/Popayán/San José del Palmar run JS-loaded or empty templates, manual-only). `web/src/app/api/cron/gov-news-check` fetches each daily, flags albergue/acopio keyword hits that co-occur with earthquake context, and stages them straight into the existing `PendingAidPoint` queue (reusing `/admin/moderacion`, no new schema) — deduped against any prior finding regardless of review outcome, so a rejected false positive doesn't reappear. Manizales upgraded to its real RSS feed (`centrodeinformacion.manizales.gov.co/feed/`, confirmed live) instead of HTML-regex scraping — structured per-article data, so findings there link to the specific article rather than a raw text window; Pereira/Cali confirmed to have no equivalent feed, stay HTML-scraped. (b) a public "suggest an update" form feeding the same queue, since locals will always out-pace any polling schedule. | Automation sweep: daily. Community submissions: real-time, moderated before publish. |
| **4 — third-party aggregators** | mapadelterremoto.com | Built 2026-08-14: `web/src/app/api/cron/aggregator-check` fetches each tracked municipio's page and counts occurrences of its two point-type labels that map to our ACOPIO/ALBERGUE kinds ("Punto de ayuda o acopio", "Albergue o punto de atención" — the only two of ~15 labels on the page that correspond to AidPoint kinds we track; everything else is damage/security/news reports, out of scope). Alert-only, same as tier-2 — a label count is a rough proxy, not confirmed data, so nothing auto-writes to AidPoint. Not a primary source, always re-verify before citing. Site has promised an open-format export after 2026-11-30 — worth switching to bulk ingestion then. | Weekly (Mondays) during active response |
